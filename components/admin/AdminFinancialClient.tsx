...
import type { GrowthFundStats } from "@/lib/platform/core/growthFund";
...
function StatusPill({ status }: { status: string }) {
  ...
  return (
    <span
      className={`text-[10px] uppercase tracking-wider font-semibold px-2 py-0.5 rounded-full border ${styles}`}
    >
      {status}
    </span>
  );
...
export default function AdminFinancialClient() {
  const [overview, setOverview] = useState<FinancialStatusOverview | null>(null);
  const [recent, setRecent] = useState<RecentPayoutRow[]>([]);
  const [growthFund, setGrowthFund] = useState<GrowthFundStats>({
    balanceCents: 0,
    ...
    async function load() {
      try {
        ...
        const data = (await res.json()) as {
          overview?: FinancialStatusOverview | null;
          recent?: RecentPayoutRow[];
          growthFund?: GrowthFundStats;
          warning?: string;
          error?: string;
        };
        if (!res.ok) {
          setWarning(data.error ?? "Could not load financial data.");
          return;
        }
        ...
        setWarning("Could not load financial data.");
      } finally {
        setLoading(false);
      }
    }
  ...
  const squares = overview?.squaresPayouts ?? {
    pending: 0,
    completed: 0,
    failed: 0,
    queued: 0,
  };
  const pickem = overview?.pickemPayouts ?? {
    pending: 0,
    completed: 0,
    failed: 0,
    queued: 0,
  ...
  return (
    <div className="max-w-5xl space-y-8">
      ...
      {warning ? (
        <LandingGlassCard className="p-4 border border-amber-500/30">
      ...
      {!loading ? (
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <AdminStatCard
              label="Pick'em Completed"
              value={pickem.completed}
              icon={CheckCircle2}
              accent="success"
            />
            <AdminStatCard label="Pick'em Failed" value={pickem.failed} icon={XCircle} />
            <AdminStatCard
              label="Pick'em Pending"
              value={pickem.pending}
              icon={Activity}
              accent="purple"
            />
          </div>
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            <LandingGlassCard className="p-5">
              ...
              <p className="text-2xl font-bold text-white">
                {overview?.webhookEvents.total24h ?? 0}
              </p>
            ...
            <LandingGlassCard className="p-5">
              ...
              <p className="text-2xl font-bold text-emerald-400">
                {formatMoney(growthFund.balanceCents)}
              </p>
              <p className="text-xs text-sb-muted mt-1">
